...
// Timeline System - Define all animations declaratively
const TIMELINE = {
    phone: {
        position: [
        ...
        rotation: [
            { time: 0, value: { x: 0, y: Math.PI, z: 0 } },       // Back
            { time: 0.1, value: { x: 0, y: Math.PI, z: 0 } },     // Hold
            { time: 0.13, value: { x: 0, y: Math.PI * 2, z: 0 } },// Rotate front
            { time: 0.16, value: { x: 0, y: Math.PI * 2, z: 0 } },// Hold
            { time: 0.2, value: { x: 0, y: Math.PI * 3, z: 0 } },  // Spin back
    ...
    // Screen replacement animation system
    screen: {
        // Controls how much of the replacement is revealed from bottom (0->1)
        // 0 = no reveal, 1 = fully revealed
        curtainReveal: [
            { time: 0, value: 0 },         // No reveal
            { time: 0.29, value: 0 },      // Hold - no reveal
            { time: 0.3, value: 0 },       // Start reveal
            { time: 0.35, value: 1 },      // Fully revealed
            { time: 1.0, value: 1 }        // Stay revealed for image 2
        ],
        firstOpacity: [
            { time: 0, value: 0 },
            { time: 0.3, value: 0 },
        ...
        secondCurtainReveal: [
            { time: 0, value: 0 },
            { time: 0.45, value: 0 },      // Start second reveal at 45%
            { time: 0.49, value: 1 },      // Fully revealed by 49%
            { time: 1.0, value: 1 }
        ],
        secondOpacity: [
            { time: 0, value: 0 },
        ...
        // Third reveal: Transact -> Reward
        thirdCurtainReveal: [
            { time: 0, value: 0 },
            { time: 0.602, value: 0 },     // Start third reveal at 60.2%
            { time: 0.645, value: 1 },     // Fully revealed by 64.5%
            { time: 1.0, value: 1 }
        ],
        thirdOpacity: [
            { time: 0, value: 0 },
        ...
        // Fourth reveal: Horizontal image from left to right
        fourthCurtainReveal: [
            { time: 0, value: 0 },
            { time: 0.80, value: 0 },     // Start at 80%
            { time: 0.85, value: 1 },     // Fully revealed by 85%
            { time: 1.0, value: 1 }
        ],
        fourthOpacity: [
            { time: 0, value: 0 },
    ...
    // Multi-text system with individual text configurations
    texts: [
        {
            id: 'engage',
...
// Timeline interpolation utility
...
// Hook to get animated values from timeline
function useTimelineValue(keyframes, scrollProgress) {
    return interpolateKeyframes(keyframes, scrollProgress);
}

// Dynamic Lighting Component - Now uses timeline system with comprehensive coverage
function DynamicLights({ scrollProgress }) {
    ...
    return (
        <>
            {/* High ambient for overall illumination */}
            ...
            {/* FRONT LIGHTING - Main screen illumination */}
            <directionalLight
                position={[0, 5, 10]}
            ...
            {/* BACK LIGHTING - For when phone shows back */}
            <directionalLight
                position={[0, 5, -12]}
            ...
            {/* TOP LIGHTING - For horizontal rotations */}
            <directionalLight
                position={[0, 15, 0]}
            ...
            {/* BOTTOM LIGHTING - Fill from below */}
            <directionalLight
                position={[0, -10, 5]}
            ...
            {/* DIAGONAL CORNER LIGHTS - For complete coverage during complex rotations */}
            <directionalLight
                position={[8, 8, 8]}
            ...
            {/* SPOT LIGHTS - Key highlights for iPhone aesthetic */}
            <spotLight
                position={[0, 10, 10]}
                angle={0.4}
                penumbra={0.3}
            ...
            {/* POINT LIGHTS - Omni-directional fill */}
            <pointLight
                position={[0, 0, 8]}
                intensity={0.5 + (lightIntensity * 0.6)}
                color="#ffffff"
...
function AnimatedIPhone({ url = "/models/iphone15pro.glb", scrollProgress = 0 }) {
    ...
    // Load both textures for screen replacement animation
    ...
    return (
        <group ref={groupRef}>
            ...
            {/* Original Screen - Always visible in background */}
            <mesh
                position={[0, 0, -0.2]} // Z position on the front of the phone (negative Z)
                rotation={[0, Math.PI, 0]} // Rotated 180 degrees to face the correct direction
            >
                <planeGeometry args={[2.2, 4.6]} /> {/* Adjusted for iPhone 15 Pro screen dimensions at scale 30 */}
                <meshBasicMaterial
                    map={originalTexture}
                    transparent
                    opacity={1} // Always fully visible
                    toneMapped={false} // Prevents tone mapping for accurate colors
                />
            </mesh>

            {/* First Replacement Screen - Curtain reveal using clipping plane */}
            <mesh
                position={[0, 0, -0.21]} // Same position as original, slightly forward
            ...
            >
                ...
                <meshBasicMaterial
                    ...
                    toneMapped={false}
                    // Use clipping plane to reveal from bottom
                    clippingPlanes={[
                        new THREE.Plane(
                            new THREE.Vector3(0, -1, 0), // Normal pointing up
                            -2.3 + (4.6 * curtainReveal) // Distance from origin, moves up as reveal increases
                        )
                    ]}
                />
            </mesh>

            {/* Second Replacement Screen - Curtain reveal using clipping plane */}
            <mesh
                position={[0, 0, -0.22]} // Slightly further forward
                rotation={[0, Math.PI, 0]}
            >
                <planeGeometry args={[2.2, 4.6]} />
                <meshBasicMaterial
                    ...
                    clippingPlanes={[
                        new THREE.Plane(
                            new THREE.Vector3(0, -1, 0), // Normal pointing up
                            -2.3 + (4.6 * secondCurtainReveal) // Distance from origin, moves up as reveal increases
                        )
                    ]}
                />
            </mesh>

            {/* Third Replacement Screen - Reward reveal over Transact */}
            <mesh
                position={[0, 0, -0.23]} // Slightly further forward
                rotation={[0, Math.PI, 0]}
            >
                <planeGeometry args={[2.2, 4.6]} />
                <meshBasicMaterial
            ...
            {/* Fourth Replacement Screen - Horizontal, reveal left to right */}
            <mesh
                position={[0, 0, -0.24]} // Slightly further forward
                rotation={[0, Math.PI, Math.PI / 2]} // Additional 90 degree rotation to orient horizontal image correctly
            >
                <planeGeometry args={[4.582, 2.291]} /> {/* Swapped width and height for horizontal orientation */}
                <meshBasicMaterial
                    ...
                    clippingPlanes={[
                        new THREE.Plane(
                            new THREE.Vector3(-1, 0, 0), // Normal pointing right to reveal left->right
                            -2.3 + (4.6 * fourthCurtainReveal) // Adjusted for the new dimensions
                        )
                    ]}
                />
            </mesh>
...
// Multi-Text Component System with Advanced Customizations
...
// Individual Text Section Component
function TextSection({ textConfig, scrollProgress }) {
    ...
    // Check if this text should be visible
    const isActive = scrollProgress >= timeRange.start && scrollProgress <= timeRange.end;

    // Get animated values for this specific text
    ...
    // Don't render if not active and opacity is 0
    ...
    // Fixed width for better text wrapping
...
// Legacy ScrollText wrapper for compatibility
function ScrollText({ scrollProgress }) {
    return <MultiScrollText scrollProgress={scrollProgress} />;
}

// Timeline Debug Component (optional - for development)
function TimelineDebug({ scrollProgress }) {
    const [showDebug, setShowDebug] = useState(false); // Always show debug by default
    ...
    // Screen replacement animation debug values
    const curtainReveal = useTimelineValue(TIMELINE.screen.curtainReveal, scrollProgress);
    const firstOpacity = useTimelineValue(TIMELINE.screen.firstOpacity, scrollProgress);
    const secondCurtainReveal = useTimelineValue(TIMELINE.screen.secondCurtainReveal, scrollProgress);
    const secondOpacity = useTimelineValue(TIMELINE.screen.secondOpacity, scrollProgress);

    // Find active text and get its opacity
...
// Main iPhone Animation Component
